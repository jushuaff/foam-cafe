// IMPORTANT:
// Business information should be verified with Foam Coffee
// before production deployment.

export const businessInfo = {
  name: "Foam Coffee",
  city: "Baguio City, Philippines",
  email: "foamcoffeeph@gmail.com",
  phone: "0977 602 3750",
  facebook: "@foamcoffeeph",
  instagram: "@foamcoffeeph",
  instagramUrl: "https://www.instagram.com/foamcoffeeph/",
  facebookUrl: "https://www.facebook.com/foamcoffeeph",
  summary: "Crafting coffee with expertise.",
  description:
    "Discover Foam Coffee in Baguio City. Explore signature coffee, pastries and all-day meals, and find the Travelite and Legarda branches.",
};

export const navigationItems = [
  { label: "Home", href: "#top" },
  { label: "Our Story", href: "#story" },
  { label: "Menu", href: "#menu" },
  { label: "Experience", href: "#experience" },
  { label: "Locations", href: "#locations" },
  { label: "Contact", href: "#contact" },
];

export const branches = [
  {
    id: "travelite",
    name: "Foam Coffee – Travelite",
    label: "Travelite / Shuntug",
    highlight: "24/7",
    hours: "OPEN 24 HOURS\nDaily",
    address: "Upper Ground Floor,\nTravelite Hotel,\nShuntug Road,\nBaguio City, Benguet",
    tel: "0977 602 3750",
    mapUrl:
      "https://www.google.com/maps?q=Travelite+Hotel+Baguio+Shuntug+Road&output=embed",
    directionsUrl:
      "https://www.google.com/maps/search/?api=1&query=Upper+Ground+Floor+Travelite+Hotel+Shuntug+Road+Baguio+City+Benguet",
    shortAddress: "Upper Ground Floor, Travelite Hotel, Shuntug Road",
  },
  {
    id: "legarda",
    name: "Foam Coffee + Roastery – Legarda",
    label: "Legarda / Bukaneg",
    highlight: "Coffee + Roastery",
    hours: "7:00 AM – 1:00 AM\nDaily",
    address: "15 Bukaneg Street,\nBaguio City, Benguet",
    tel: "0977 602 3750",
    mapUrl:
      "https://www.google.com/maps?q=15+Bukaneg+Street+Baguio+City+Benguet&output=embed",
    directionsUrl:
      "https://www.google.com/maps/search/?api=1&query=15+Bukaneg+Street+Baguio+City+Benguet",
    shortAddress: "15 Bukaneg Street, Baguio City",
  },
];

export const menuHighlights = [
  {
    category: "SIGNATURE COFFEE",
    items: ["Foam Signature Latte", "Speculoos Latte", "Tiramisu Latte", "Sakura Latte"],
  },
  {
    category: "MATCHA & NON-COFFEE",
    items: ["Apple Pie Iced Tea", "Toast Matcha", "Smoothies", "House-made refreshments"],
  },
  {
    category: "PASTRIES",
    items: ["Artisan donuts", "Fresh bakes", "Croissant-style pastries", "Seasonal sweets"],
  },
  {
    category: "ALL-DAY MEALS",
    items: ["Korean Bulgogi Tapa", "Smashed Burger", "Comfort food plates", "Late-night bites"],
  },
  {
    category: "PASTA & SAVORY",
    items: ["Bulgogi Pasta", "Doritos Chicken", "Savory comfort bowls", "Coffeehouse favorites"],
  },
  {
    category: "DESSERTS",
    items: ["Dessert plates", "Sweet endings", "Pastry pairings", "Seasonal specials"],
  },
];

export const experiences = [
  {
    title: "Coffee Tasting",
    description:
      "Slow and curious tasting sessions for coffee lovers who want to explore flavor, origin and brewing notes.",
  },
  {
    title: "Barista Workshop",
    description:
      "Hands-on learning for café culture, espresso basics, and building better daily coffee routines.",
  },
  {
    title: "Reservations",
    description:
      "A simple way to plan a coffee catch-up, study session, or casual meeting before you arrive.",
  },
  {
    title: "Franchise Consultation",
    description:
      "For brand-minded conversations around café concepts, coffee culture and operational direction.",
  },
];

export const socialGallery = [
  { id: "foam-01", alt: "Coffee in a warm cafe setting", src: "/images/places/coffee-closeup.jpg" },
  { id: "foam-02", alt: "Cafe interior with tables and natural light", src: "/images/places/travelite-interior.jpg" },
  { id: "foam-03", alt: "Fresh pastry selection", src: "/images/places/pastries.jpg" },
  { id: "foam-04", alt: "Coffee shop exterior atmosphere", src: "/images/places/travelite-exterior.jpg" },
  { id: "foam-05", alt: "Prepared cafe food", src: "/images/places/food.jpg" },
  { id: "foam-06", alt: "Coffee and cafe atmosphere", src: "/images/places/roastery.jpg" },
];

export const imageSlots = {
  hero: "/images/places/hero.jpg",
  signature: "/images/places/signature.jpg",
  traveliteExterior: "/images/places/travelite-exterior.jpg",
  traveliteInterior: "/images/places/travelite-interior.jpg",
  legardaExterior: "/images/places/legarda-exterior.jpg",
  legardaInterior: "/images/places/legarda-interior.jpg",
  roastery: "/images/places/roastery.jpg",
  coffeeCloseup: "/images/places/coffee-closeup.jpg",
  pastries: "/images/places/pastries.jpg",
  food: "/images/places/food.jpg",
  mood: "/images/places/baguio-mood.jpg",
};

export const socialLinks = {
  facebook: "https://www.facebook.com/foamcoffeeph",
  instagram: "https://www.instagram.com/foamcoffeeph/",
};

export const seoKeywords = [
  "Foam Coffee Baguio",
  "Baguio cafe",
  "coffee shop Baguio",
  "24 hour cafe Baguio",
  "specialty coffee Baguio",
  "Legarda cafe",
  "Baguio coffee shop",
];
